...
function CheckCart({ carts, matches }) {
  return (
    <Fragment>
      <Typography variant="body2" fontSize={"24px"} lineHeight={"30px"} fontWeight={600} sx={{ padding: "24px 24px 0px", marginBottom: "24px" }}>
        Sản phẩm
      </Typography>
      {matches ? (
        <TableContainer>
        ...
        carts?.map((item, index) => (
          <Stack key={index} direction={"row"} alignItems={"center"} spacing={2} p={1} m={2} sx={{ border: "1px solid #ccc", borderRadius: "12px" }}>
